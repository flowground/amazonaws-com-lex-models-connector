# ![LOGO](logo.png) Amazon Lex Model Building Service **flow**ground Connector
...
### GetBot
<blockquote><p>Returns metadata information for a specific bot. You must provide the bot name and the bot version or alias. </p> <p> This operation requires permissions for the <code>lex:GetBot</code> action. </p></blockquote>

#### Input Parameters
* `name` - _required_
* `versionoralias` - _required_
* `X-Amz-Content-Sha256` - _optional_
* `X-Amz-Date` - _optional_
* `X-Amz-Algorithm` - _optional_
* `X-Amz-Credential` - _optional_
...
### DeleteBotVersion
<blockquote><p>Deletes a specific version of a bot. To delete all versions of a bot, use the <a>DeleteBot</a> operation. </p> <p>This operation requires permissions for the <code>lex:DeleteBotVersion</code> action.</p></blockquote>
...
**flow**ground :- Telekom iPaaS / amazonaws-com-lex-models-connector<br/>
